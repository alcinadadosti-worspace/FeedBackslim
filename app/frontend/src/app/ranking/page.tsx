...
export default function RankingPage() {
  ...
  const ColaboradorInitial = ({ nome, size = 'md' }: { nome: string; size?: 'md' | 'lg' | 'xl' }) => {
    ...
    return (
      <div className={`${sizes[size]} bg-neutral-200 border-2 border-neutral-900 flex items-center justify-center font-bold text-neutral-700`}>
  ...
  return (
    <SmartLayout>
      <div className="max-w-4xl mx-auto">
        ...
        {/* Tabs */}
        <div className="flex gap-2 mb-8 p-1 bg-neutral-100 border-2 border-neutral-200">
          <button
                    ...
                    {gestores.map((gestor, index) => (
                      ...
                      >
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-xs text-neutral-500 mt-1">{gestor.totalAvaliacoes} avaliações</p>
                        </div>
                      </Link>
                    ))}
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                  <div className="md:order-1 md:mt-8">
                    <Card className="text-center bg-neutral-50">
                      ...
                      <h3 className="font-bold text-lg mt-3">{colaboradores[1].nome}</h3>
                      <div className="mt-3"><SimpleRating value={colaboradores[1].mediaFeedback} /></div>
                      <p className="text-xs text-neutral-500 mt-1">{colaboradores[1].totalFeedbacks} feedbacks</p>
                    </Card>
                  </div>
                  <div className="md:order-2">
                    <Card className="text-center bg-yellow-50 border-yellow-400">
                      ...
                      <Trophy className="w-8 h-8 text-yellow-500 mx-auto mt-2" />
                      <h3 className="font-bold text-xl mt-2">{colaboradores[0].nome}</h3>
                      <div className="mt-3"><SimpleRating value={colaboradores[0].mediaFeedback} size="lg" /></div>
                      <p className="text-xs text-neutral-500 mt-1">{colaboradores[0].totalFeedbacks} feedbacks</p>
                    </Card>
                  </div>
                  <div className="md:order-3 md:mt-8">
                    <Card className="text-center bg-amber-50">
                      ...
                      <h3 className="font-bold text-lg mt-3">{colaboradores[2].nome}</h3>
                      <div className="mt-3"><SimpleRating value={colaboradores[2].mediaFeedback} /></div>
                      <p className="text-xs text-neutral-500 mt-1">{colaboradores[2].totalFeedbacks} feedbacks</p>
                    </Card>
                  </div>
                </div>
              )}
                ...
                <CardContent>
                  <div className="space-y-3">
                    {colaboradores.map((col, index) => (
                      <div
                        key={col.slackId}
                        className="flex items-center gap-4 p-4 border-2 border-neutral-200"
                      >
                        <span className={`w-10 h-10 border-2 border-neutral-900 flex items-center justify-center font-bold ${getMedalColor(index)}`}>
                          {index + 1}
                        </span>
                        ...
                        <div className="flex-1">
                          <p className="font-bold text-neutral-900">{col.nome}</p>
                          <p className="text-sm text-neutral-500">Colaborador</p>
                        </div>
                        <div className="text-right">
                          <SimpleRating value={col.mediaFeedback} />
                          <p className="text-xs text-neutral-500 mt-1">{col.totalFeedbacks} feedbacks</p>
                        </div>
                      </div>
                    ))}
                  </div>
