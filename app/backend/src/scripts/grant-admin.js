...
// Uso (de dentro de app/backend): node src/scripts/grant-admin.js

// require() é sequencial — dotenv carrega antes do firebase-admin ser usado
...
async function main() {
  ...
  const snap = await db.collection('users').where('email', 'in', EMAILS_TO_PROMOTE).get();
  ...
  const notFound = EMAILS_TO_PROMOTE.filter(e => !snap.docs.some(d => d.data().email === e));
  if (notFound.length) console.warn('Não encontrados no Firestore:', notFound.join(', '));
